...
struct HourlyForecastView: View {
    ...
    private var forecastIndices: Range<Int> {
        let startIndex = findCurrentHourIndex()
    ...
    private func hourlyItemView(for index: Int) -> some View {
        // We still need the start index to know if this item is "Now"
        let startIndex = findCurrentHourIndex()
        
        VStack(spacing: 12) {
            // Time Label
            Text(index == startIndex ? "Now" : formatTime(from: hourlyData.time[index]))
                .fontWeight(.semibold)
            
            // Weather Icon
            Image(systemName: WeatherCodeMapper.symbol(
                for: hourlyData.weatherCode[index] ?? 0)
            )
